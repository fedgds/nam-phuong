    ...
    <section class="main-content">
        <div class="gallery-tab">
            ...
            <?php foreach ($gallery as $key => $category) : ?>
                <div class="tab-item tab-gallery" data-view="view-<?= $key ?>">
                    <?= $category['category'] ?>
                </div>
            <?php endforeach;?>
        </div>
        ...
        <?php if (count($all_images) > 12) : ?>
            <button class="load-more button-primary" data-target="view-all" data-offset="12" id="load-more-all">Load More</button>
        <?php endif; ?>
        
        <!-- View Category -->
        <?php foreach ($gallery as $key => $category) : ?>
        <div id="view-<?= $key ?>" class="image-gallery" data-total="<?= count($category['image']); ?>" data-offset="0">
        ...
        <?php if (count($category['image']) > 12) : ?>
            <button class="load-more button-primary" data-target="view-<?= $key ?>" data-category="<?= $key ?>" data-offset="12">Load More</button>
        <?php endif; ?>
    <?php endforeach; ?>
        ...
        jQuery('#load-more-all').on('click', function ($) {
            ...
            jQuery.ajax({
                ...
                success: function (response) {
                    if (response) {
                        jQuery('#view-all').append(response);
                        button.data('offset', offset + 12);

                        const totalImages = jQuery('#view-all').data('total');
